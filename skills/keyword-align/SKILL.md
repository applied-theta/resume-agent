...
A job description is **required** for this command. The JD can be provided as a file in `{workspace}/{slug}/input/`, pasted inline in the conversation, as a URL, or via the `--jd` argument.
...
```
/keyword-align                                              → scans {workspace}/{slug}/input/ for resume and JD
...
1. **`--jd` file path**: The user provided a file path via `--jd`. Copy it to `{workspace}/{slug}/input/`: `mkdir -p {workspace}/{slug}/input && cp <provided-path> {workspace}/{slug}/input/`. If the copy fails, report the error and stop.
...
3. **`--jd` URL**: The user provided a URL via `--jd` or inline. Use `WebFetch` to retrieve the content.
   ...
   - Stop execution.
4. **File**: Look for a JD file in `{workspace}/{slug}/input/`. Search for files with names containing "jd", "job", or "description" (case-insensitive), or `.txt` files that are not the resume.
...
> 1. **File**: Place a job description file in `{workspace}/{slug}/input/` (name it with "jd", "job", or "description" in the filename)
...
2. If no resume file is found in `{workspace}/{slug}/input/`:
   - Display a clear message to the user:
     ...
     > 2. **Workspace**: Place a PDF or Markdown file in `{workspace}/{slug}/input/` and run `/keyword-align`
   - Stop execution.
...
- The job description content (inline text, fetched URL content, or the file path in `{workspace}/{slug}/input/`)
...
> Keyword alignment analysis encountered an issue. Please check the session directory for details: `{workspace}/{slug}/sessions/{session}/`
